 <?php
        ...
        if(isset($_SESSION['signed_in'])!=true){
         ...
         $id=$_SESSION['docid'];
        $query="SELECT Count(postID) as totalposts FROM doctor,posts WHERE docid=user_id AND user_id='$id'";
         ?>
...
<?php
                                    $mysqli = new mysqli('localhost', 'root', '', 'modernhospital');
                                    $query="SELECT * FROM doctor,posts WHERE docid=user_id ORDER BY postID desc";
                                    $data = mysqli_query($mysqli,$query);
                                    $p = 2;
                                         while($row = mysqli_fetch_assoc($data)) {
                                          echo("ggggg");
                                        $p++;
                                    $content=$row['postsCont'];
