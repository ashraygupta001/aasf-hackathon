...
$mysqli = new mysqli('localhost', 'root', '', 'modernhospital');
 if(isset($_POST['submit'])){
     $username = $_SESSION['docname'];
     $password = md5($_POST['currentpassword']);
     $newpassword = md5($_POST['newpassword']);
     $confirmnewpassword = md5($_POST['confirmnewpassword']);
    $sql  ="SELECT password FROM doctor WHERE docname='$username'";
    $data = mysqli_query($mysqli, $sql);
     ...
     if($password==$oldpassword){
            if($newpassword==$confirmnewpassword){
                $querychange=mysqli_query($mysqli,"UPDATE doctor SET docpass='$newpassword' WHERE docname='$username'");
            }
            else{
